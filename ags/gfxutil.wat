(module
  (func $ensure_memory_reach (param $end i32)
    (block $enough
      ;; get page delta and do nothing if it's <= 0
      (br_if $enough
        (i32.le_s
          ;; transform $end from byte offset to page delta relative to current maximum page
          (tee_local $end (i32.sub (current_memory) (i32.div_u (get_local $end) (i32.const 65536))))
          (i32.const 0)
        )
      )
  ...
  (func $expand_r5g6b5 (param $r5g6b5 i32) (param $opaque i32) (param $transparent_r5g6b5 i32) (result i32)
    ...
    ;; alpha compo3nt
    (block $set_alpha
      (br_if $set_alpha (i32.and (i32.eqz (get_local $opaque)) (i32.ne (get_local $r5g6b5) (get_local $transparent_r5g6b5))))
  ...
  (func $expand_b8g8r8 (param $b8g8r8 i32) (param $opaque i32) (param $transp i32) (result i32)
    (block $with_alpha
      (br_if $with_alpha (i32.and (i32.eqz (get_local $opaque) (i32.eq (get_local $b8g8r8) (get_local $transp)))))
      (return (i32.or (get_local $b8g8r8) (i32.const -16777216))) ;; 0xff000000
    )
    (return (i32.and (get_local $b8g8r8) (i32.const 16777215))) ;; 0x00ffffff
  )
  ...
  (func (export "unpack_allegro") (param $out i32) (param $in i32) (param $in_end i32) (result i32)
    ...
    (loop $top
      (block $end_check
        (br_if $end_check (i32.lt_u (get_local $in) (get_local $in_end)))
        (return (get_local $out))
      )
      (set_local $count (i32.load8_s (get_local $in)))
      (set_local $in (i32.add (get_local $in) (i32.const 1)))
      ;; if count>=0, it's a literal copy of (count+1) bytes
      (block $literal
        ;; break if count is negative
        (br_if $literal (i32.lt_u (get_local $count) (i32.const 0)))
        ;; add 1 to count
      ...
      ;; if count=-127, copy a single byte
      (block $onechar
        (br_if $onechar (i32.ne (get_local $count) (i32.const -127)))
        (i32.store8 (get_local $out) (i32.load8_u (get_local $in)))
        (set_local  $in (i32.add (get_local  $in) (i32.const 1)))
        (set_local $out (i32.add (get_local $out) (i32.const 1)))
        (br $top)
      )
      ;; count<0: copy one byte of input 1+abs(count) times
